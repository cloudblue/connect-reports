...
def test_generate_csv_renderer(progress, client_factory, response_factory, billing_request):
    ...
    assert result[0] == list(map(lambda item: item.replace(' ', '_').lower(), HEADERS))
    assert len(result[0]) == 26
    assert result[0][0] == 'billing_request_id'
    assert len(result) == 2
    assert progress.call_count == 1
    assert progress.call_args == ((2, 1),)
...
def test_generate_json_renderer(progress, client_factory, response_factory, billing_request):
    ...
    assert len(result) == 1
    assert len(result[0]) == 26
    assert result[0][0] == {'billing_request_id': 'BRV-3970-9735-4579-0002'}
    assert progress.call_count == 1
