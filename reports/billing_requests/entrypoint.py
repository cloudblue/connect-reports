...
def generate(
    ...
    if renderer_type == 'csv':
        yield list(map(lambda item: item.replace(' ', '_').lower(), HEADERS))
        progress = 1
    else:
        progress = 0
    ...
    for request in requests:
        ...
        if renderer_type == 'json':
            yield _process_json_line(request, connection)
        else:
...
def _process_json_line(request, connection):
    std_headers = list(map(lambda item: item.replace(' ', '_').lower(), HEADERS))
    return [{std_headers[idx]: item} for idx, item in enumerate(_process_line(request, connection))]
